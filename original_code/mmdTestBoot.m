...
%Arthur Gretton
%07/12/08
% Modified by Xinlu Pan 26/4/19
% Modification: Changed biased V-statistic to unbiased U-statistic
% to match the description in Gretton et al. 2012, Section 5.
...
%%%%%%% I revised here to use unbiased
term1 = (sum(K(:))-trace(K))/(m*(m-1));
term2 = (sum(L(:))-trace(L))/(m*(m-1));
term3 = 2*(sum(KL(:)))/(m*m);

testStat =1/m * (term1 + term2 - term3);
...
%  disp(strcat('Generating new threshold: ',threshFileName))
  ...
  for whichSh=1:params.shuff
    ...
    MMDarr(whichSh) =1/m * (term1 + term2 - term3);
